...
const Threads: React.FC<ThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    // 1920은 원본 컴포넌트 기본값이었으나, 고해상도/고주사율 모니터에서
    // 매 프레임 픽셀 셰이더(라인 40개 x Perlin 노이즈 2회)를 그 해상도로
    // 계속 돌리면 GPU 가속이 약한 환경(또는 소프트웨어 렌더링)에서
    // CPU 사용률이 100%에 가깝게 치솟는 문제가 있어 낮췄다. 은은한
    // 배경 효과라 해상도를 좀 낮춰도 시각적으로 거의 차이가 없다.
    ...
    // 은은하게 움직이는 배경 장식일 뿐이라 60fps로 그릴 필요가 없다.
    // 30fps로만 그려도 눈으로는 거의 구분이 안 되면서 GPU 제출/셰이더
    // 실행 횟수를 절반으로 줄여 CPU/GPU 부담을 낮춘다.
    const TARGET_FPS = 30;
